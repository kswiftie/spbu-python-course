...
from utils import (
    check_isolation,
    check_evaluation,
    check_evaluation_with_isolation
)
...
@pytest.mark.parametrize(
    "function, args, kwargs, expected",
    [
        (smart_args(check_evaluation_with_isolation), [1, {"a": 10, "b": 11}, 150], {},
         ([1, {"a": 10, "b": 11}, 150], {}, 150)),
        (smart_args(check_evaluation_with_isolation), [1], {"d": {"a": 2, "b": 3}, 'y': 152},
         ([1], {'d': {"a": 2, "b": 3}, 'y': 152}, 152)),
        (smart_args(check_evaluation_with_isolation), [], {"d": {"a": 8.6, "b": -4.2}, 'y': 6},
         ([], {'d': {"a": 8.6, "b": -4.2}, 'y': 6}, 6)),
    ],
)
def test_smartargs_Evaluation_substitutions_with_Isolation(function, args, kwargs, expected):
...
@pytest.mark.parametrize(
    ...
    [
        (smart_args(check_evaluation_with_isolation), [1, {"a": 10, "b": 11}], {}, 3,
         ([1, {"a": 10, "b": 11}], {}, 3)),
        (smart_args(check_evaluation_with_isolation), [1], {'d': {"a": 2, "b": 3}}, 5,
         ([1], {'d': {"a": 2, "b": 3}}, 5)),
        (
                smart_args(check_evaluation_with_isolation), [], {"d": {"a": 8.6, "b": -4.2}}, 7,
                ([], {'d': {"a": 8.6, "b": -4.2}}, 7)),
    ],
)
def test_smartargs_Evaluation_uniqueness_with_Isolation(function, args, kwargs, launch_cnt, expected):
    y = set()
    for _ in range(launch_cnt):
